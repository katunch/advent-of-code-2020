...
public class Day2Solver {
    ...
    public func solve(input: String) {
        ...
        let validPasswordCount = lines.reduce(0) { (result, line) -> Int in
            if isPasswordValid(line: String(line)) { return result + 1 }
            return result
        }
        
        print("We have \(validPasswordCount) valid passwords")
    }
    ...
    public func solve2(input: String) {
        ...
        let validPasswordCount = lines.reduce(0) { (result, line) -> Int in
            if isPatternValid(line: String(line)) { return result + 1 }
            return result
        }
    ...
    private func isPasswordValid(line: String) -> Bool {
        let splits = line.split(separator: ":")
        let policy = splits.first?.trimmingCharacters(in: .whitespacesAndNewlines)
        let password = splits[1].trimmingCharacters(in: .whitespacesAndNewlines)
        
        let policySplits = policy?.split(separator: " ")
        let minAndMaxValues = policySplits?.first?.split(separator: "-")
        let minOccurence = Int(minAndMaxValues!.first!)!
        let maxOccurrence = Int(minAndMaxValues!.last!)!
        let letterWhichPasswordMustContain = String(policySplits!.last!)
        
        let occurenceCount = password.filter{ String($0) == letterWhichPasswordMustContain }.count
        
        return minOccurence...maxOccurrence ~= occurenceCount
    }
    
    private func isPatternValid(line: String) -> Bool {
        let splits = line.split(separator: ":")
        let policy = splits.first?.trimmingCharacters(in: .whitespacesAndNewlines)
        let password = splits[1].trimmingCharacters(in: .whitespacesAndNewlines)
        
        let policySplits = policy?.split(separator: " ")
        let indexesToMatch = policySplits!.first!.split(separator: "-").map { Int($0)! }
        let letterWhichPasswordMustContain = String(policySplits!.last!)
        
        var matchesPolicy = false
        for i in indexesToMatch {
            let characterAtIndex = String(password[password.index(password.startIndex, offsetBy: i-1)])
            if characterAtIndex == letterWhichPasswordMustContain {
                if matchesPolicy { matchesPolicy = false }
                else { matchesPolicy = true }
            }
        }
