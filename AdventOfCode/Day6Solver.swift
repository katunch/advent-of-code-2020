...
//  AdventOfCode
//
//  Created by katunch on 09.12.20.
//
...
public class Day6Solver {
    ...
    public func solve(input: String) {
        let groupInputs = input.components(separatedBy: "\n\n")
        let sum = groupInputs.map { numberOfUniqueCharacters(inGroup: $0) }.reduce(0) { (result, number) -> Int in
            return result + number
        }
        print("The Sum of all yes answers is: \(sum)")
    }
    ...
    public func solve2(input: String) {
        ...
        let sum = groupInputs.map { numberOfYesAnswers(inGroup: $0) }.reduce(0) { (result, number) -> Int in
            return result + number
        }
        
        print("The sum of everyones yes answers is: \(sum)")
    }
    
    private func numberOfUniqueCharacters(inGroup input: String) -> Int {
        var uniqueCharacters: Set<String> = []
        input.replacingOccurrences(of: "\n", with: "").forEach { (c) in
            uniqueCharacters.insert(String(c))
        }
        return uniqueCharacters.count
    }
    
    private func numberOfYesAnswers(inGroup input: String) -> Int {
        var uniqueChars: Set<String> = []
        input.replacingOccurrences(of: "\n", with: "").forEach { uniqueChars.insert(String($0)) }
        
        let personInputs = input.components(separatedBy: "\n")
        
        var numberOfYesAnswers = 0
        uniqueChars.forEach { (char) in
            var containsYes = true
            personInputs.forEach { (personInput) in
                if containsYes {
                    containsYes =  personInput.contains(char)
                }
            }
            if containsYes {
...
public extension String {
    func countOccurrencesOf(_ search: String, minCount: Int = 1) -> Int {
        let count = filter { String($0) == search }.count
        if count > minCount {
            return count
        }
        return 0
