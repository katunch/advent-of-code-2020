...
public class Day4Solver {
    ...
    typealias PassportProperties = [String: String]
    ...
    public func solve(input: String) {
        ...
        print("we have to check \(passports.count) passports...")
        
        let numberOfValidPassports = passports.reduce(0) { (result, passport) -> Int in
            return result + (isPassportValid(passport: passport) ? 1: 0)
        }
        
        print("We found \(numberOfValidPassports) valid passports.")
    ...
    public func solve2(input: String) {
        ...
        let numberOfValidPassports = passports.reduce(0) { (result, passport) -> Int in
            return result + (isPassportDataValid(passport: passport) ? 1: 0)
        }
        
        print("We found \(numberOfValidPassports) valid passports.")
    }
    
    private func isPassportValid(passport: String) -> Bool {
        
        let searchFor: [String] = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]
        
        let numberOfFindings = searchFor.reduce(0) { (result, elementToSearch) -> Int in
            if passport.contains(elementToSearch) {
                return result + 1
            }
            return result
        }
        ...
        return numberOfFindings == searchFor.count
    }
    
    private func isPassportDataValid(passport: String) -> Bool {
        if !isPassportValid(passport: passport) {
            return false
        }
        
        let simplifiedPassport = passport.replacingOccurrences(of: "\n", with: " ")
        let props = simplifiedPassport.components(separatedBy: " ")
        var passportProps: PassportProperties = [:]
        
        props.forEach { (property) in
            let components = property.components(separatedBy: ":")
            passportProps[components.first!] = components.last!
        }
        
        
        return isByrValid(input: passportProps) &&
            isIyrValid(input: passportProps) &&
            isEyrValid(input: passportProps) &&
            isHgtValid(input: passportProps) &&
            isHclValid(input: passportProps) &&
            isEclValid(input: passportProps) &&
            isPidValid(input: passportProps)
        
    }
    
    
    private func isByrValid(input: PassportProperties) -> Bool {
        guard let byr = input["byr"] else { return false }
        guard let value = Int(byr) else { return false }
        return 1920...2002 ~= value
    }
    
    private func isIyrValid(input: PassportProperties) -> Bool {
        guard let iyr = input["iyr"] else { return false }
        guard let value = Int(iyr) else { return false }
        return 2010...2020 ~= value
    }
    
    private func isEyrValid(input: PassportProperties) -> Bool {
        guard let eyr = input["eyr"], let value = Int(eyr) else { return false }
        return 2020...2030 ~= value
    }
    
    private func isHgtValid(input: PassportProperties) -> Bool {
        guard let hgt = input["hgt"] else { return false }
        if (hgt.hasSuffix("cm")) {
            guard let height = Int(hgt.replacingOccurrences(of: "cm", with: "")) else { return false }
            return 150...193 ~= height
        }
        else if (hgt.hasSuffix("in")) {
            guard let height = Int(hgt.replacingOccurrences(of: "in", with: "")) else { return false }
            return 59...76 ~= height
        }
        else {
            return false
    ...
    private func isHclValid(input: PassportProperties) -> Bool {
        guard let hcl = input["hcl"] else { return false }
        
        do {
            let pattern = try NSRegularExpression(pattern: "^#([0-9A-F]{3}){1,2}$", options: NSRegularExpression.Options.caseInsensitive)
            let match = pattern.firstMatch(in: hcl, options: [], range: NSRange(location: 0, length: hcl.utf16.count))
            if match != nil {
                return true
            }
            return false
    ...
    private func isEclValid(input: PassportProperties) -> Bool {
        guard let ecl = input["ecl"] else { return false }
        return ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].contains(ecl)
    }
    
    private func isPidValid(input: PassportProperties) -> Bool {
        guard let pid = input["pid"], pid.count == 9 else { return false }
        let nf = NumberFormatter()
        guard let _ = nf.number(from: pid) else { return false }
        return true
    }
}
